...
async def chat_stream_generator(request: ChatRequest, user_id: str | None = None):
    """
    ...
    try:
        ...
        try:
            memory_ctx = get_memory_context(request.message, user_id=user_id)
            if memory_ctx:
        ...
        try:
            ...
            msg_count = count_messages(request.thread_id, user_id=user_id)
            
            if msg_count == 0:
                title = await generate_title(request.message)
                save_thread(request.thread_id, title, user_id=user_id)
            else:
                update_thread_timestamp(request.thread_id, user_id=user_id)

            save_message(str(uuid4()), request.thread_id, "user", request.message, user_id=user_id)
            save_message(str(uuid4()), request.thread_id, "assistant", full_response, user_id=user_id)
        except Exception as e:
...
@router.post("/chat")
async def chat_endpoint(request: ChatRequest, x_user_id: str | None = Header(None)):
    """
    ...
    return StreamingResponse(
        chat_stream_generator(request, user_id=x_user_id),
